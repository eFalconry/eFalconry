  ...
  <section style={{ background: '#0F1218', borderTop: '1px solid rgba(255,255,255,0.063)', borderBottom: '1px solid rgba(255,255,255,0.063)', padding: '100px 0' }}>
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 2rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 1, background: 'rgba(255,255,255,0.063)', border: '1px solid rgba(255,255,255,0.063)', borderRadius: 18, overflow: 'hidden', marginBottom: '5rem' }}>
        {([['0.05s','The time a visitor takes to form a first impression of your website — and decide if you are worth trusting.'],['75%','Of consumers judge a business credibility directly from its website design. Not what you offer — how you look.'],['3x','More leads for businesses with professional, modern websites compared to outdated ones.']] as [string,string][]).map(([num,text]) => (
          <div key={num} style={{ background: '#0F1218', padding: '3rem 2.5rem' }}>
            <div style={{ fontFamily: 'Bebas Neue,cursive', fontSize: 76, lineHeight: 0.9, color: '#F07A20', marginBottom: '1rem' }}>{num}</div>
            <div style={{ fontSize: 14, color: '#7C8494', lineHeight: 1.6, maxWidth: 210 }}>{text}</div>
          </div>
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: '4rem', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ fontSize: 16, color: '#7C8494', lineHeight: 1.72, marginBottom: '1.25rem' }}>A modern website signals competence. It answers questions, builds confidence, and converts visitors into paying customers. An outdated site does the opposite — it drives them straight to a competitor.</p>
